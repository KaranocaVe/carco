...
namespace carco.Services
{
    public class AnalyticsService
    {
        ...
        var rows2 = await _db.Set<VSalesDetail>().AsNoTracking()
            ...
            .GroupBy(s => new { Y = s.SaleDate.Year, M = s.SaleDate.Month })
            .Select(g => new { g.Key.Y, g.Key.M, Units = g.Count(), Revenue = g.Sum(x => x.SalePrice) })
        ...
        return new ModelBestMonth { ModelName = modelName, Month = new DateTime(row.Y, row.M, 1), Units = row.Units, Revenue = row.Revenue };
        }
