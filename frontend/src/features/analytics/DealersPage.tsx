...
export default function DealersPage() {
  ...
  return (
    <Stack spacing={3}>
      <PageHeader title="经销商分析" actions={<DateRangePicker start={start} end={end} onChange={(s, e) => { setStart(s); setEnd(e) }} />} />
      {dwellQ.isLoading ? <KpiCardSkeleton /> : (dwellQ.data && (
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 6 }}>
          {topRevenueQ.isLoading ? <ChartSkeleton /> : <BarTop title="经销商金额Top" items={(topRevenueQ.data ?? []).map((x) => ({ name: x.dealerName, value: x.revenue }))} />}
        </Grid>
        <Grid size={{ xs: 12, lg: 6 }}>
          {topUnitsQ.isLoading ? <ChartSkeleton /> : <BarTop title="经销商销量Top" items={(topUnitsQ.data ?? []).map((x) => ({ name: x.dealerName, value: x.units }))} />}
        </Grid>
      </Grid>
    </Stack>
  )
